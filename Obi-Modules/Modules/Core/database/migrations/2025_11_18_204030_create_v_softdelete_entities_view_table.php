...
return new class extends Migration
{
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 1     -- case
       AND ul.event_id = 3     -- delete_case
       AND (
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 8     -- accident_type
       AND ul.event_id = 24    -- delete_accident_type
       AND (
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 2     -- customer
       AND ul.event_id = 6     -- delete_customer
       AND (
            CAST(JSON_UNQUOTE(JSON_EXTRACT(ul.details, '$.before.id')) AS UNSIGNED) = c.id
         OR CAST(JSON_UNQUOTE(JSON_EXTRACT(ul.details, '$.after.id'))  AS UNSIGNED) = c.id
       )
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 4     -- bank
       AND ul.event_id = 12    -- delete_bank
       AND (
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 5     -- insurer
       AND ul.event_id = 15    -- delete_insurer
       AND (
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 6     -- loss_adjuster
       AND ul.event_id = 18    -- delete_loss_adjuster
       AND (
    ...
    LEFT JOIN grupoint_obi_users.user_logs ul
        ON ul.model_id = 3     -- user
       AND ul.event_id = 9     -- delete_user
       AND (
