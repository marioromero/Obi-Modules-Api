...
{
    /** La vista se crea en la conexión 'cases_db' (QA). */
...
SELECT
  ...
  /* ───── Agregados JSON (históricos completos) ───── */
  sl.step_logs_json,
  cfj.case_flows_json
...
LEFT JOIN grupoint_traro_QA.users cons ON cons.id = c.consultant_id AND cons.role_id = 5
LEFT JOIN grupoint_traro_QA.users asg  ON asg.id  = c.assigned_user
LEFT JOIN grupoint_traro_QA.users crt  ON crt.id  = c.created_by
LEFT JOIN grupoint_traro_QA.users aag  ON aag.id  = cu.assigned_agent
...
LEFT JOIN (
  SELECT cf1.*
  FROM grupoint_traro_QA.case_flows cf1
  JOIN (
    SELECT obi_case_id, MAX(id) AS max_id
    FROM grupoint_traro_QA.case_flows
...
LEFT JOIN grupoint_traro_QA.users lusr ON lusr.id = csl.last_state_change_user_id
...
/* ─────────── JSON de TODOS los case_flows (campos permitidos) ─────────── */
LEFT JOIN (
  SELECT
    cf.obi_case_id,
    JSON_ARRAYAGG(
      JSON_OBJECT(
        'id', cf.id,
        'crm_caso_id', cf.crm_caso_id,
        'obi_case_id', cf.obi_case_id,
        'fecha_documentos_listos', cf.fecha_documentos_listos,
        'contrato_enviado_a_acepta', cf.contrato_enviado_a_acepta,
        'fecha_firma_contrato', cf.fecha_firma_contrato,
        'mandato_enviado_a_acepta', cf.mandato_enviado_a_acepta,
        'fecha_firma_mandato', cf.fecha_firma_mandato,
        'numero_notificaciones_documentos_enviados', cf.numero_notificaciones_documentos_enviados,
        'numero_notificaciones_documento_pendiente', cf.numero_notificaciones_documento_pendiente,
        'notificacion_documento_firmado', cf.notificacion_documento_firmado,
        'active_notifications', cf.active_notifications,
        'user_id', cf.user_id
      ) ORDER BY cf.id
    ) AS case_flows_json
  FROM grupoint_traro_QA.case_flows cf
  GROUP BY cf.obi_case_id
) cfj ON cfj.obi_case_id = c.id;
SQL);
    }
